...
static int cp210x_write_u16_reg(struct usb_serial *serial, u8 req, u16 val)
{
	...
	return usb_control_msg(serial->dev, usb_sndctrlpipe(serial->dev, 0),
			...
			cur_altsetting->desc.bInterfaceNumber, NULL, 0,
			USB_CTRL_SET_TIMEOUT);
}

static void cp210x_usb_reset_device(struct usb_serial *serial)
{
	int result;
	result = cp210x_write_u16_reg(serial, CP210X_IFC_ENABLE, UART_ENABLE);
	if (result < 0) 
                printk("failed set request 0x%x status: %d\n", CP210X_IFC_ENABLE, result);
	result = cp210x_write_u16_reg(serial, CP210X_PURGE, PURGE_ALL);
	if (result < 0) 
                printk("failed set request 0x%x status: %d\n", CP210X_PURGE, result);
	result = cp210x_write_u16_reg(serial, CP210X_IFC_ENABLE, UART_DISABLE);
	if (result < 0) 
                printk("failed set request 0x%x status: %d\n", CP210X_IFC_ENABLE, result);
}
...
static int cp210x_startup(struct usb_serial *serial)
{
	...
	cp210x_usb_reset_device(serial);	
	return 0;
}
